...
float_t minRawValue = pwmRangeStep * 65; 
float_t rawStep = (maxRawValue - minRawValue) / 100; // вычисляем 1% рабочего диапазона raw
...
uint32_t getLedDuty(const uint16_t raw) {
  ...
  // если больше чем минимум
    ...
    retVal = ((raw - minRawValue) / rawStep) * workPwmRangeStep + minDutyLedLight;
